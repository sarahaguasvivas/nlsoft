...
NNP = NeuralNetworkPredictor(model_file = model_filename,
                    ...
                    Q =     np.array([[50., 0.],
                                     [0., 70.]]),
                    Lambda = np.array([[1e-4, -5e-8],
                                       [-5e-8, 0.07]]),
                        y0 = [0.0, 0.0, 0.0],
                        u0 = [0.0, 0.0], s = 1e-20, b = 1e-3, r = 4e5)
...
try:
    for e in range(NUM_EXPERIMENTS):
        ...
        for n in range(NUM_TIMESTEPS):
            ...
            u_optimal, del_u,  _ = NR_opt.optimize(u = np.array(u_optimal_old), delu = del_u, \
